...
def plot_loss(train_loss_values):
    ...
    plt.plot(train_loss_values, color='red', label='Train Loss')
    plt.legend()
...
def train(env_para):
    ...
    file_path1 = f'./data_folder/{data_name}'
    ...
    filtered_data = filtered_data0 + filtered_data1  # 或者使用 extend()

    print("拓展后的训练数据长度", len(filtered_data))

    batch_size = 32
    train_loader = DataLoader(filtered_data, batch_size=batch_size, shuffle=True)
    # 创建模型
    ...
    models_dir = "./SL_models"
    os.makedirs(models_dir, exist_ok=True)  # 如果文件夹不存在，则创建
    torch.save(model.state_dict(), f'SL_models/model-{env_para}.pth')
...
if __name__ == '__main__':
    ...
    random.seed(seed)
    np.random.seed(seed)
    env_para = input("please input env para：")

    a = input("train?")
    if a == 'y':
        train(env_para)
    # """
    env_list = ['triangle_insertion.xml']
    results=[]
    for k in range(len(env_list)):
        ...
        # 加载保存的模型权重
        model.load_state_dict(torch.load(f'SL_models/model-{env_para}.pth'))
        # 设置模型为评估模式
        model.eval()
        ...
        for i in range(1, test_steps+1):
            ...
            for j in range(attempt_time):
                print(f"Current step {i}: SL-Test success rate = {(success_cnt / i) * 100} %")
            ...
            # 进行前向传播
                ...
                if done:
                    ...
                    with open(log_filename, 'a', newline='') as csvfile:
                        csv_writer = csv.writer(csvfile)
                        csv_writer.writerow(
                            [i, j + 1, 1 if infos["success"] else 0, success_cnt / i * 100])
                    env.interface.disconnect()
                    if i != test_steps:
                        env.generate_initial_pose()
                        obs, sl_action = sample_reset()
