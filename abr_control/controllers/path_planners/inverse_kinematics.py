...
class InverseKinematics:
    ...
    def __init__(self, robot_config, max_dx=0.2, max_dr=2 * np.pi, max_dq=2*np.pi):
    ...
    def generate_path(
        ...
        # print(transformations.quaternion_from_euler(
        #             target_position[3],
        #             target_position[4],
        #             target_position[5],
        #             axes="sxyz",))
        # print(target_position[3:])
        # Qd = np.array(transformations.unit_vector(target_position[3:]))

        q = np.copy(position)
        # limit max step size in operational space
        max_dq = self.max_dq * dt*10
    ...
    def next(self):
        ...
        self.n = min(self.n + 2, self.n_timesteps)
