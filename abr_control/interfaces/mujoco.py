...
class Mujoco(Interface):
    ...
    def connect(self, joint_names=None, camera_id=-1):
        ...
        if self.display_frequency > 0:
            self.viewer = mujoco_viewer.MujocoViewer(self.model, self.data, self.im, self.sv)
    ...
    def close_gripper(self):
        self.data.qpos[6] = 0.8
        self.data.ctrl[6] = 255
        mujoco.mj_forward(self.model, self.data)

        for _ in range(300):
            self.viewer.render()

    def set_target_gripper(self, current_target_joint_values):
        self.data.qpos[6] = current_target_joint_values
        # mujoco.mj_forward(self.model, self.data)
        target_ctrl = math.ceil(current_target_joint_values*255/0.8)

        self.data.ctrl[6] =target_ctrl
        mujoco.mj_forward(self.model, self.data)
