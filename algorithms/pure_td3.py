...
class TD3Agent:
    ...
    def test(self, cfg):
        ...
        # 加载检查点
        model_save_dir = "./RL_models/12-19-2024-23-55-35-he00/models/"
        ...
        agent = TD3(128, self.render_env.action_space.shape[0], float(self.render_env.action_space.high[0]),
                    feature_extractor)
        # 将提取的对象加载到你的模型和相关对象中
        agent.load(os.path.join(model_save_dir, 'model_iter55_reward-32.2'))
    ...
    def play_once(self, actor_critic, ob_rms=None, stochastic=False, mode='loop', render=True, record=False):
        with torch.no_grad():
            ...
            while not done:
                ...
                # recurrent_hidden_states = torch.zeros((1, self.actor_critic.recurrent_hidden_state_size))
                _, action, _, recurrent_hidden_states = actor_critic.act(ob, recurrent_hidden_states,
                                                                         torch.ones((1, 1)),
    ...
    def play(self, cfg):
        ...
        while num_games != games_cnt:
            reward, success, improve_episode_cnt, episode_len, extra_info = self.play_once(self.actor_critic, ob_rms,
                                                                                           cfg['params']['general'][
                                                                                               'stochastic'], \
                                                                                           'loop',
                                                                                           cfg['params']['general'][
        ...
        print_info(
            '[Summary] Avg reward = {:.3f}, Success rate = {:.2f}%, Avg success episode length = {:.2f}, Improve rate = {:.2f}%'.format(
                total_reward / games_cnt, success_cnt / games_cnt * 100., episode_len_sum / success_cnt,
                improve_cnt / np.sum(class_cnt) * 100.))
        ...
        for class1 in range(3):
            for class2 in range(3):
                ...
                print_info(
                    'Class [{}, {}], total cnt = {}, success rate = {:.3f}%, improve rate = {:.3f}%'.format(class1,
                                                                                                            class2,
                                                                                                            class_cnt[
                                                                                                                class1][
                                                                                                                class2],
