...
class TD3Agent:
    def __init__(self, cfg):
        ...
        self.target_domain_file = os.path.join("data_folder",
                                               self.env_name.split("_")[0] + f"_data_{self.timestamp}.pkl")
    ...
    def train(self):
        ...
        for j in range(0, num_updates):
            ...
            if ((sum(success_history) / len(success_history)) > best_success_rate
                    or ((sum(success_history) / len(success_history)) >= best_success_rate and np.mean(
            ...
            if j % self.log_interval == 0 and len(episode_rewards) > 1:
                ...
                self.writer.add_scalar('success_rate/step', (sum(success_history) / len(success_history)) * 100.,
                                       total_num_steps)
                self.writer.add_scalar('success_rate/time', (sum(success_history) / len(success_history)) * 100.,
                                       time_elapse)
            ...
            if (self.log_interval is not None and self.log_interval > 0 and (j + 1) % self.log_interval == 0):
                ...
                fp_log.write(
                    'curriculum stage = {}, success_cnt = {}, num_steps = {}, time = {}, mean(len) = {:.1f}, min(len) = {}, max(len) = {}, mean(reward) = {:.3f}, min(reward) = {:.3f}, max(reward) = {:.3f}, eval = {:.3f}, success_rate = {:.1f} \n'.format(
                        ...
                        reward_mean, reward_min, reward_max, last_eval_rew, (sum(success_history) / len(
                            success_history)) * 100.))
                fp_log.close()
    ...
    def test(self, cfg):
        ...
        # 加载检查点
        model_save_dir = "./nopre_DARL_models/01-13-2025-10-45-26-he00/models/"
        checkpoint1 = torch.load(os.path.join(model_save_dir, "feature_extractor_iter50_loss0.7.pt"))
        # create TD3 agent
        agent = TD3(128, self.render_env.action_space.shape[0], float(self.render_env.action_space.high[0]),
                    feature_extractor)
        # 将提取的对象加载到你的模型和相关对象中
        agent.load(os.path.join(model_save_dir, 'model_iter55_reward-65.0'))
