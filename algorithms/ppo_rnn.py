...
class TD3Agent:
    def __init__(self, cfg):
        ...
        self.env_para = input("please input env para：")
        ...
        self.pretrained_model = model.TacTorModel(self.tac_input_channels, self.torque_input_channels, self.tactile_hidden_size,
        ...
        # 加载特征提取器部分全参数
        self.feature_extractor = model.FeatureExtractor(self.tac_input_channels,self.torque_input_channels,self.tactile_hidden_size,
                                                        self.torque_hidden_size)
        # self.feature_extractor.cnn = self.pretrained_model.cnn
        # self.feature_extractor.tactile_lstm = self.pretrained_model.tactile_lstm
        # self.feature_extractor.torque_lstm = self.pretrained_model.torque_lstm
        # self.feature_extractor.fc_cnn = self.pretrained_model.fc_cnn
        # self.feature_extractor.dropout_cnn = self.pretrained_model.dropout_cnn
        ...
        # self.actor_critic.actor_net.mean_net.load_state_dict(self.pretrained_model.fc_layers.state_dict())

        self.actor_critic.to(self.device)
        discriminator = model.Discriminator()
        self.adversarial_trainer = model.AdversarialTrainer(self.feature_extractor, discriminator)
        # 创建一个时间戳用于文件命名
        self.timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")  # 使用 datetime.now()
        self.target_domain_file = os.path.join("data_folder", self.env_name.split("_")[0]+f"_data_{self.timestamp}.pkl")
        self.adv_update_frequency = cfg['params']['network']['adv_update_frequency']
    ...
    def train(self):
        ...
        episode_lens = deque(maxlen=20)
        success_history = deque([0] * 20, maxlen=20)
        num_updates = int(
        ...
        for j in range(0, num_updates):
            ...
            for step in range(self.num_steps):
                ...
                with torch.no_grad():
                    ...
                    action = action + np.random.normal(0, args.exploration_noise, size=self.render_env.action_space.shape[0])
                ...
                save_data(self.render_env.info_buf['success'], next_state, infos['real_action'], self.target_domain_file)
                ...
                self.agent.memory.push((state[0], state[1], next_state[0],next_state[1], action, reward, np.float64(done)))
            ...
            if len(self.agent.memory.storage) >= self.num_steps:
                self.agent.update(10, self.writer)

            if ((j) % self.adv_update_frequency == 0 or j == num_updates -1):
                ...
                # 执行对抗网络的更新
                mean_feature_extractor_loss = []  # 记录了特征提取器的损失值
                mean_discriminator_loss = []  # 记录了判别器的损失值

                for epoch in range(self.adv_num_epochs):
                    # 遍历源域数据加载器
                    for batch_idx, (source_batch, target_batch) in enumerate(combined_loader):
                        global_step = (
                                (j // self.adv_update_frequency) * self.adv_num_epochs * num_batches +
                                epoch * num_batches +
                                batch_idx)
                        # for target_batch in target_loader:
                        #     # 使用对抗训练器更新模型
                        discriminator_loss, feature_extractor_loss = self.adversarial_trainer.update(source_batch, target_batch)
                        ...
                        mean_discriminator_loss.append(discriminator_loss)

                        self.writer.add_scalar("Adversarial/Discriminator_Loss", discriminator_loss, global_step)
                        self.writer.add_scalar("Adversarial/Feature_Extractor_Loss", feature_extractor_loss,
                                               global_step)

                torch.save(self.feature_extractor.state_dict(),
                    os.path.join(model_save_dir, 'feature_extractor_iter{}'.format(j) + "_loss{:.1f}".format(np.mean(mean_feature_extractor_loss)) + ".pt"))
            ...
            if (j % self.save_interval == 0 or j == num_updates - 1):
                self.agent.save(os.path.join(model_save_dir,
                                             'model_iter{}'.format(j) + "_reward{:.1f}".format(np.mean(episode_rewards))))
            if ((sum(success_history) / len(success_history)) > best_success_rate
                    or ((sum(success_history) / len(success_history)) >= best_success_rate and np.mean(episode_lens) < best_len)):
                    # best_eval_rew = np.mean(episode_rewards)
                    best_success_rate = sum(success_history) / len(success_history)
            ...
            if j % self.log_interval == 0 and len(episode_rewards) > 1:
                ...
                if ((sum(success_history) / len(success_history)) > best_success_rate
                    ...
                    print_info(
                        ...
                        .format(j, total_num_steps,
                                ...
                                (sum(success_history) / len(success_history)) * 100.))
                ...
                self.writer.add_scalar('success_rate/step', (sum(success_history) / len(success_history)) * 100., total_num_steps)
                self.writer.add_scalar('success_rate/time', (sum(success_history) / len(success_history)) * 100., time_elapse)
                self.writer.add_scalar('success_rate/iter', (sum(success_history) / len(success_history)) * 100., j)
            ...
            if (self.log_interval is not None and self.log_interval > 0 and (j + 1) % self.log_interval == 0):
                ...
                fp_log.write('curriculum stage = {}, success_cnt = {}, num_steps = {}, time = {}, mean(len) = {:.1f}, min(len) = {}, max(len) = {}, mean(reward) = {:.3f}, min(reward) = {:.3f}, max(reward) = {:.3f}, eval = {:.3f}, success_rate = {:.1f} \n'.format(
                    self.render_env.stage, success_cnt, total_num_steps, end - start, len_mean, len_min, len_max, reward_mean, reward_min, reward_max, last_eval_rew, (sum(success_history) / len(success_history)) * 100.))
                fp_log.close()
    ...
    def test(self,cfg):
        ...
        # env.seed(self.seed)
        model_num = input("please input model num：")
        # tac_obs_shape = (5, 2, 4, 4)
        # tor_obs_shape = (32, 3)
        actor_critic_fn = getattr(model, cfg['params']['network']['actor_critic'])
        ...
        feature_extractor = model.FeatureExtractor(self.tac_input_channels, self.torque_input_channels, self.tactile_hidden_size,
                                                        self.torque_hidden_size)
        feature_extractor.to(self.device)

        # feature_extractor = self.feature_extractor
        # 加载检查点
        model_save_dir = "./DARL_models/01-06-2025-13-57-02-tr00-1/models/"
        checkpoint1 = torch.load(os.path.join(model_save_dir, "feature_extractor_iter50_loss0.8.pt"))
        # create TD3 agent
        agent = TD3(128, self.render_env.action_space.shape[0], float(self.render_env.action_space.high[0]),
                         feature_extractor)
        # 将提取的对象加载到你的模型和相关对象中
        agent.load(os.path.join(model_save_dir,'model_iter52_reward-43.7'))
        ...
        log_filename = os.path.join(log_dir, f"{self.timestamp}_{self.env_para}_DARL_{model_num}.csv")
        ...
        for i in range(0, test_steps):
            ...
            for j in range(attempt_time):
                ...
                print(f"第{i+1}/{test_steps}轮测试-第{j+1}/{attempt_time}次尝试-当前成功率{success_cnt / (i+0.0001) * 100}%")
    ...
    def play_once(self, actor_critic, ob_rms=None, stochastic=False, mode='loop', render=True, record=False):
        with torch.no_grad():
