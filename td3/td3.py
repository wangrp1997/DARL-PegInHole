...
parser.add_argument('--play', action='store_true',help='play a stored policy')
...
class Replay_buffer():
    ...
    def sample(self, batch_size):
        ...
        for i in ind:
            X0,X1, Y0,Y1, U, R, D = self.storage[i]
            x0.append(np.array(X0, copy=False))
            x1.append(np.array(X1, copy=False))
            y0.append(np.array(Y0, copy=False))
            y1.append(np.array(Y1, copy=False))
            u.append(np.array(U, copy=False))
            r.append(np.array(R, copy=False))
            d.append(np.array(D, copy=False))

        return np.array(x0),np.array(x1), np.array(y0), np.array(y1),np.array(u), np.array(r).reshape(-1, 1), np.array(d).reshape(-1, 1)
...
class Actor(nn.Module):
    ...
    def forward(self, state):
        # state = state.to(dtype=torch.float64)
        # a = F.relu(self.fc1(state))
        # a = F.relu(self.fc2(a))
        # a = torch.tanh(self.fc3(a)) * self.max_action
        a = self.mean_net(state)
        return a
...
class TD3():
    def __init__(self, state_dim, action_dim, max_action, feature_extractor):

        self.actor = Actor(state_dim, action_dim, max_action).to(device)
        self.actor_target = Actor(state_dim, action_dim, max_action).to(device)
        # self.actor.mean_net.load_state_dict(pretrain_model.fc_layers.state_dict())
    ...
    def select_action(self, state):
        state = state.reshape(1, -1).double().clone().detach().to(device)
        return self.actor(state).cpu().data.numpy().flatten()

    def update(self, num_iteration, writer):
        ...
        for i in range(num_iteration):
            x0,x1, y0,y1, u, r, d = self.memory.sample(args.batch_size)
            state0 = torch.DoubleTensor(x0).to(device)
            state0 = torch.squeeze(state0, dim=1)
            state1 = torch.DoubleTensor(x1).to(device)
            state1 = torch.squeeze(state1, dim=1)

            action = torch.FloatTensor(u).to(device)
            next_state0 = torch.DoubleTensor(y0).to(device)
            next_state0 = torch.squeeze(next_state0, dim=1)
            next_state1 = torch.DoubleTensor(y1).to(device)
            next_state1 = torch.squeeze(next_state1, dim=1)

            done = torch.FloatTensor(d).to(device)
            reward = torch.FloatTensor(r).to(device)

            state = self.feature_extractor(state0,state1)
            next_state = self.feature_extractor(next_state0,next_state1)
            ...
            writer.add_scalar('Loss/Q1_loss', loss_Q1, global_step=self.num_critic_update_iteration)
            ...
            writer.add_scalar('Loss/Q2_loss', loss_Q2, global_step=self.num_critic_update_iteration)
